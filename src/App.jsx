...
function BackBtn({ onClick }) {
  return <button onClick={onClick} style={{ ...S.btn, background: "none", color: T.green, fontSize: 14, padding: 0, marginBottom: 16 }}>← Назад</button>;
}
...
// ── Навігація ───────────────────────────────────────────────────────────────
const NAV = [["market", "🏪", "Маркет"], ["my", "🛒", "Мої"], ["qr", "📱", "QR"], ["seller", "📊", "Бізнес"], ["wallet", "💰", "Гаманець"]];

function Nav({ tab, setTab }) {
  return (
    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 64, background: T.card, borderTop: `1px solid ${T.border}`, ...S.flex, zIndex: 100 }}>
      {NAV.map(([t, icon, label]) => (
        <button key={t} onClick={() => setTab(t)} style={{ ...S.btn, flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 2, background: "transparent" }}>
          <span style={{ fontSize: 20 }}>{icon}</span>
          <span style={{ fontSize: 9, fontWeight: 700, color: tab === t ? T.green : T.textSec }}>{label}</span>
          {tab === t && <div style={{ width: 16, height: 2, background: T.green, borderRadius: 1 }} />}
        </button>
      ))}
    </div>
  );
    ...
    <div onClick={() => onOpen(deal)} style={{ ...S.card, borderRadius: 20, overflow: "hidden", cursor: "pointer", borderColor: isIn ? T.accent + "44" : T.border, boxShadow: deal.hot ? "0 4px 20px rgba(249,115,22,0.08)" : "0 1px 4px rgba(0,0,0,0.04)" }}>
      ...
      <div style={{ padding: "10px 14px 14px" }}>
        <div style={{ ...S.flex, gap: 8, marginBottom: 10 }}>
          ...
          {isIn && <div style={{ width: 28, height: 28, background: T.accent, borderRadius: "50%", ...S.flex, justifyContent: "center", color: "#fff", fontSize: 14 }}>✓</div>}
        </div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${T.greenLight}, ${T.greenBorder})`, padding: "24px 16px 20px", marginBottom: 16 }}>
        <div style={{ ...S.flex, justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: T.green }}>Малий бізнес — великі можливості</div>
          </div>
    ...
    <div style={{ paddingBottom: 100 }}>
      <div style={{ background: `linear-gradient(180deg, ${T.greenLight}, ${T.card})`, padding: "20px 16px 24px" }}>
        ...
        <div style={{ ...S.card, background: T.greenLight, borderColor: T.greenBorder, ...S.flex, gap: 12 }}>
          ...
          <div style={{ textAlign: "center" }}><div style={{ fontSize: 10, color: T.textSec }}>Перевірений</div><div style={{ fontSize: 20 }}>✅</div></div>
        </div>
      </div>
        ...
        {!isIn && (
          <div style={S.card}>
            ...
            <div style={{ ...S.flex, gap: 16, justifyContent: "center" }}>
              <button onClick={() => setQty(Math.max(deal.min, qty - 1))} style={{ ...S.btn, width: 40, height: 40, borderRadius: T.radiusSm, background: T.cardAlt, color: T.text, fontSize: 20 }}>−</button>
              <span style={{ fontSize: 24, fontWeight: 900, color: T.text, width: 60, textAlign: "center" }}>{qty}</span>
              <button onClick={() => setQty(Math.min(deal.max, qty + 1))} style={{ ...S.btn, width: 40, height: 40, borderRadius: T.radiusSm, background: T.accent, color: "#fff", fontSize: 20 }}>+</button>
            </div>
      ...
      <div style={{ position: "fixed", bottom: 64, left: 0, right: 0, padding: "12px 16px", background: T.card, borderTop: `1px solid ${T.border}`, zIndex: 50 }}>
        {isIn ? (
          <div style={{ ...S.flex, gap: 10 }}>
            <div style={{ flex: 1, background: T.greenLight, border: `1px solid ${T.accent}44`, borderRadius: 14, padding: 14, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11, color: T.green, marginTop: 2 }}>Повідомимо коли наберуть</div>
            </div>
            <button onClick={() => onBuy(deal, qty)} style={{ ...S.btn, background: "#6366f1", color: "#fff", borderRadius: 14, padding: "0 16px", fontSize: 13 }}>📱 QR</button>
          </div>
        ) : (
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ ...S.card, textAlign: "center", padding: 24 }}>
        ...
        <div style={{ fontSize: 18, fontWeight: 900, color: T.text, marginBottom: 4 }}>{code}</div>
...
function QRHub() {
  return (
    <div style={S.page}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ ...S.card, ...S.flex, gap: 16, padding: 20 }}>
          <Icon emoji="📱" size={56} />
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: T.text }}>Покупець</div>
            <div style={{ fontSize: 12, color: T.textSec }}>Покажи QR продавцю для отримання товару</div>
          </div>
        </div>
        <div style={{ ...S.card, ...S.flex, gap: 16, padding: 20 }}>
          <Icon emoji="📷" size={56} />
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: T.text }}>Продавець</div>
            <div style={{ fontSize: 12, color: T.textSec }}>Скануй QR покупця для підтвердження видачі</div>
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  function renderContent() {
    ...
    switch (tab) {
      case "market": return <MarketPage joined={joined} onJoin={onJoin} onOpen={onOpen} />;
